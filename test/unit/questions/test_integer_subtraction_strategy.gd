...
var strategy: IntegerSubtractionStrategy


func before_each() -> void:
	strategy = IntegerSubtractionStrategy.new()
...
func test_question_text_matches_operands_and_correct_answer() -> void:
	...
	assert_eq(result["correct_answer"], a - b, "correct_answer should equal the actual difference of the operands used")


func test_result_is_never_negative() -> void:
	for i in range(50):
		var result: Dictionary = strategy.generate(1)
		assert_gte(result["correct_answer"], 0, "subtraction results should never be negative for this age group")
...
func test_operand_size_scales_with_difficulty() -> void:
	...
	assert_gt(high_max, low_max, "higher difficulty should be able to produce larger results than low difficulty")
